...
import { LastStepSignal } from "../../Test/TestView";
...
export default class PokerStateView extends BaseView {
  ...
  unuse() {
    ...
    this.Back.end();
    this.Front.end();
    this.Front.onScaleChanged(cc.v2(0, 1), 0, () => {});
    this.Back.onScaleChanged(cc.v2(1, 1), 0, () => {});
  }
  ...
  turnFront(
    ...
    this.Back.end();
    this.Front.end();
    this.Back.onScaleChanged(
      cc.v2(0, 1),
      lastTime,
      () => {
        this.Front.onScaleChanged(cc.v2(1, 1), lastTime, callback, 0);
      },
      delay
    );
  }
