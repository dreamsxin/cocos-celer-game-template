...
export default class NumberChangedView extends BaseView {
    ...
    onNumberChanged(num: number, callback?: Function) {
        ...
        if (this.num != this.showNum) {
            ...
            this.step = step > 0 ? Math.ceil((step) / this.STEP) : Math.floor((step) / this.STEP);
    ...
    update(dt: number) {
        ...
        this.Label.string = this.showNum >= 0 ? this.showNum.toString() : "/" + Math.abs(this.showNum).toString();
