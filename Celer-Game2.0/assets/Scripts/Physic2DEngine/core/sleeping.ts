...
import { Manifold } from "../collision/mainfold";
...
export class Sleeping {
  ...
  afterCollision(manifolds: Manifold[]) {
    let manifold: Manifold, bodyA: Body, bodyB: Body;
    ...
    for (var i = 0; i < manifolds.length; i++) {
      ...
      if (
        bodyB.kinetic ||
        (bodyA.sleeping && bodyB.motion > this.wakeMotionThreshold)
      ) {
        this.wake(bodyA);
        continue;
      }
      ...
      if (
        bodyA.kinetic ||
        (bodyB.sleeping && bodyA.motion > this.wakeMotionThreshold)
      ) {
        this.wake(bodyB);
        continue;
      }
