...
export class Body {
  ...
  /** 方法 */
  methods: IBodyOpt["methods"];
  /** 渲染函数 */
  render: Function;

  constructor(opt: IBodyOpt, type: number) {
    this.id = Util.id();
  ...
  getInvInertia(): number {
    return this.inertia === 0 || this.static || this.kinetic
      ? 0
      : 1 / this.inertia;
  }
  ...
  applyImpulse(impulse: Vector, offset: Vector) {
    ...
    this.angularVelocity += this.invInertia * offset.cross(impulse);
  }
  ...
  applyForce(force: Vector, offset?: Vector) {
    ...
    if (offset !== undefined) {
      this.torque += offset.cross(this.force);
    }
  }
  ...
  afterRemove() {
    this.methods.afterRemove(this);
  }

  sleepStart() {
    this.methods.sleepStart(this);
  }

  sleepEnd() {
    this.methods.sleepEnd(this);
  }
