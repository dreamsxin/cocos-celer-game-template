...
export class JointConstraint {
  ...
  preSolveVelocity(joints: Joint[], dt: number) {
    for (let i = 0; i < joints.length; i++) {
      ...
      bodyA.applyImpulse(joint.jointImpulse.inverse(_tempVector4), joint.rA);
  ...
  solveVelocity(joints: Joint[]) {
    for (let i = 0; i < joints.length; i++) {
      ...
      joint.rA.crossNum(angularVelocityA, _tempVector1);
      joint.rB.crossNum(angularVelocityB, _tempVector2);
      ...
      let velocityPointA = velocityA.add(_tempVector1, _tempVector1),
        ...
        stiffnessImpulse = joint.jointImpulse.scale(1 - joint.stiffness),
        impulse = joint.jointMatrix.multiplyVec(
          velocityBias.sub(stiffnessImpulse)
        );

      joint.jointImpulse.x += impulse.x;
      joint.jointImpulse.y += impulse.y;

      bodyA.applyImpulse(impulse.inverse(_tempVector4), joint.rA);
