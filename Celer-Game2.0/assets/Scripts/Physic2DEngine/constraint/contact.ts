  ...
  preSolveVelocity(manifolds: Manifold[], dt: number) {
    ...
    for (i = 0; i < manifolds.length; ++i) {
      ...
      for (j = 0; j < collision.contacts.length; j++) {
        ...
        let bias =
          60 * Math.max(0, contact.depth - this.slop) * this.biasFactor;
        ...
        if (contact.normalImpulse !== 0 || contact.tangentImpulse !== 0) {
          ...
          p.x =
            normal.x * contact.normalImpulse +
            tangent.x * contact.tangentImpulse;
          p.y =
            normal.y * contact.normalImpulse +
            tangent.y * contact.tangentImpulse;

          bodyA.applyImpulse(p, contact.offsetA);
          bodyB.applyImpulse(p.inverse(p), contact.offsetB);
        }
      }
    }
  }
  ...
  solveVelocity(manifolds: Manifold[]) {
    ...
    for (i = 0; i < manifolds.length; i++) {
      ...
      for (j = 0; j < collision.contacts.length; j++) {
        ...
        contact.offsetA.crossNum(bodyA.angularVelocity, _tempVector1);
        contact.offsetB.crossNum(bodyB.angularVelocity, _tempVector2);
        ...
        // 计算法向冲量
        normalImpulse =
          (relativeNormalVelocity + contact.velocityBias) * contact.shareNormal;

        let frictionNormalImpulse = contact.normalImpulse;
        if (
          collision.isReuse &&
          relativeNormalVelocity < 0 &&
          relativeNormalVelocity ** 2 > this.restFactor
        ) {
        ...
        bodyB.applyImpulse(impulse.inverse(impulse), contact.offsetB);
        ...
        contact.tangentImpulse = Util.clamp(
