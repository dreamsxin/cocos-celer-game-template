...
export function axiesFilter(poly: Polygon, geometry: Body): Axis[] {
  let centroidVector = geometry.position.sub(poly.position, _tempVector1),
    axesA: Axis[],
    axesB: Axis[],
    supportIndexA: number,
    supportIndexB: number,
    i,
    res: Axis[] = [];
  ...
  if (geometry.type === BodyType.Polygon) {
    axesB = findClosestAxes(
      <Polygon>geometry,
      poly,
      centroidVector.inverse(centroidVector)
    );
...
function findClosestAxes(
  ...
  for (let i = 0; i < v.length; i++) {
    ...
    distance =
      (vertex.x - oppositeCentroid.x) ** 2 +
      (vertex.y - oppositeCentroid.y) ** 2;
