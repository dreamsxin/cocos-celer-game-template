...
function findIncidentEdge(
  oppositeVertexList: VertexList,
  normal: Vector,
  oppositeClosestIndex: number
): Edge {
  let prev: Vector, cur: Vector, next: Vector, index: number;
...
export function vClip(minOverlap: MinOverlap): Contact[] {
  ...
  incEdge = findIncidentEdge(
    <VertexList>axis.opposite,
    normal,
    minOverlap.oppositeClosestIndex
  );
  refEdge = axis.edge;

  // ------------------------------------- 首先向 refEdge 的内部进行筛选 -------------------

  let refV = refEdge.end.sub(refEdge.start).normalize(),
    refN = normal,
  ...
  removeIndex = clipSide(incEdge, refV.inverse(refV), refEdge.start.dot(refV));
  if (removeIndex !== -1 && incVertex[removeIndex]) {
...
export function vClipCircle(
  ...
  let incEdge: Edge = findIncidentEdge(polygon.vertexList, normal, null),
    vertex: Vector = circle.position.extend(normal, circle.radius - depth / 2),
