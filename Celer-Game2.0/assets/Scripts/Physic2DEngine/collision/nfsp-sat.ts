...
export class NFSP_SAT {
  ...
  polygonCollideBody(
    ...
    let canReuse: boolean = this.canReuseCollision(
        poly,
        geometry,
        prevCollision
      ),
    ...
    if (canReuse) {
      ...
      minOverlap = this.detect(
        poly,
        geometry,
        [collision.axis],
        collision.oppositeClosestIndex
      );
    ...
    // 若不能用缓存，则进行完整的测试
      ...
      collision.tangent = normal.normal();
    }
  ...
  circleCollideCircle(
    ...
    normal = this.reviseNormal(axis, circleA, circleB).normalize();
    ...
    let position = circleB.position.extend(
      normal,
      circleB.radius - minOverlap.value / 2
    );

    collision.contacts = [
      ContactConstraint.create(null, position, minOverlap.value),
    ];
    collision.collide = true;
  ...
  private detect(
    ...
    geometry: Body,
    axes: Axis[],
    prevOppositeClosestIndex?: number
  ): MinOverlap {
    let minOverlap: number = Infinity,
      oppositeClosestIndex: number,
      getOverlaps = this.enableSATBoost
        ? this.selectiveProjectionMethod
        : this.fullProjectionMethod,
  ...
  private getTestAxes(poly: Polygon, geometry: Body): Axis[] {
    ...
    if (this.enableSATBoost) {
      axes = axiesFilter(poly, geometry);
      ...
      let opposite =
          geometry.type === BodyType.Circle
            ? <Circle>geometry
            : (<Polygon>geometry).vertexList,
        oppositeAxes: Axis[],
        i;

      axes = [];

      for (i = 0; i < poly.axies.length; i++) {
        if (poly.axies[i]) {
          poly.axies[i].opposite = opposite;
          axes.push(poly.axies[i]);
        }
      }
      ...
      if (circleAxis) {
        ...
        oppositeAxes = (<Polygon>geometry).axies;
   ...
   * @param axis
   */
  private fullProjectionMethod(
    poly: Polygon,
    geometry: Body,
    axis: Axis,
    prevOppositeClosestIndex: number
  ): { depth: number; oppositeClosestIndex: number } {
    let axisVector: Vector = axis.value,
      partA = poly.vertexList,
      partB =
        geometry.type === BodyType.Polygon
          ? (<Polygon>geometry).vertexList
          : <Circle>geometry,
      projection1: { max: any; min: any },
      projection2: { min: any; max: any };
   ...
   */
  private selectiveProjectionMethod(
    poly: Polygon,
    geometry: Body,
    axis: Axis,
    oppositeClosestIndex?: number
    ...
    maxOverlap =
      supportProjection - opposite[oppositeClosestIndex].dot(axisVector);
