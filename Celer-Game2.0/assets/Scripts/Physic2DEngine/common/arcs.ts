...
export const Arcs = {
  ...
  getAxes(circle: Circle, poly: Polygon): Axis {
    let closestVertex = Vertices.getClosestVertex(
      circle.position,
      poly.vertexList
    );

    _tempDynamicAxis.value = closestVertex.sub(circle.position).normalize();
  ...
  projection(circle: Circle, axis: Vector): { min: number; max: number } {
    let len = circle.position.project(axis);
  ...
  isContains(circle: Circle, point: Vector): boolean {
    return (
      circle.radius -
        Math.hypot(point.x - circle.position.x, point.y - circle.position.y) >
      0
    );
  },
};
