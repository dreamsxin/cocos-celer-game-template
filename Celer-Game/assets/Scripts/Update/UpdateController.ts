...
export default class UpdateController extends SingleTon<UpdateController>() {
    ...
    private readonly STORAGE_PATH: string = ((window["jsb"] && jsb.fileUtils ? jsb.fileUtils.getWritablePath() : '/') + 'hot-update');
    ...
    private updateCallback(event: any) {
        ...
        if (needRestart) {
            ...
            console.log(" paths storage:", cc.sys.localStorage.getItem('select-game'))
        }
