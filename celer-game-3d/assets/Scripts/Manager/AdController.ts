import { director, Node, UITransform, VideoPlayer } from "cc";
...
export class AdController extends SingleTon<AdController>() {
  ...
  showAd(
    ...
    if (CELER_X) {
      ...
      if (director.getScene() && director.getScene().getChildByName("Canvas")) {
        ...
        let adNode = new Node("adNode");
        let videoPlayer = adNode.addComponent(VideoPlayer);
  ...
  private onAdFinish(uniqeKey: string) {
    if (this.finishCallback[uniqeKey]) {
      let callbacks: ((adUnitId: string) => void)[] =
        this.finishCallback[uniqeKey];
